...
def train(params, n_epochs, verbose=True):
    ...
    if FLAGS.filename == None:
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=n_epochs-FLAGS.warmup, eta_min=2e-5, last_epoch=-1)
        ...
        # for _ in range(50):
        #     scheduler.step()
        # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=n_epochs-FLAGS.warmup, eta_min=5e-5, last_epoch=-1)
    ...
    # metrics    
    writer = SummaryWriter(f'runs/final_exp/full_run_laploss/{name}')
    
    results = ResultStore(
        writer=writer,
        metrics = ['psnr', 'ssim', 'ie', 'L1_loss', 'lap_loss'],
        folds=FOLDS
    )
