...
def train(params, n_epochs, verbose=True):
    ...
    if params['lmd']:
        ...
        train_sampler = torch.utils.data.sampler.WeightedRandomSampler(weights_vim, FLAGS.num_train_samples, replacement=False)
    ...
    # metrics    
    writer = SummaryWriter(f'runs/final_exp/full_run_ablation/{name}')
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if epoch >= FLAGS.warmup-1 and params['lrscheduling']:
            scheduler.step()   
...
if __name__ == '__main__':
    ...
    param_combinations = [
        {'jitter_prob':0, 'flip_probs':.5, 'lmd':True, 'lrscheduling':True},
        {'jitter_prob':0, 'flip_probs':0, 'lmd':True, 'lrscheduling':True},
        {'jitter_prob':.2, 'flip_probs':.5, 'lmd':False, 'lrscheduling':True},
        {'jitter_prob':.2, 'flip_probs':.5, 'lmd':True, 'lrscheduling':False},
    ]
