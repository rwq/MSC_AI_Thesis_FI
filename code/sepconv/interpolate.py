...
import sys
sys.path.append('E:\scriptieAI\code\sepconv')
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict(torch.load('E:/scriptieAI/code/sepconv/network-' + arguments_strModel + '.pytorch'))
    # end
...
def interpolate(inputs):
    X0 = (inputs[0].permute(2,0,1) / 255.0).float()
    X1 = (inputs[1].permute(2,0,1) / 255.0).float()
    
    output = estimate(X0, X1)
    output = (output.clamp(0.0, 1.0) * 255).cpu()
...
if __name__ == '__main__':
    ...
    if arguments_strOut.split('.')[-1] in [ 'bmp', 'jpg', 'jpeg', 'png' ]:
        ...
        for intFrame, numpyFrame in enumerate(numpyFrame[:, :, ::-1] for numpyFrame in tqdm(moviepy.editor.VideoFileClip(filename=arguments_strVideo).iter_frames())):
