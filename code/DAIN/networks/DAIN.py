...
class DAIN(torch.nn.Module):
    ...
    def forward_flownets(self, model, input, time_offsets = None):
        ...
        temps = [nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False)(temp)  for temp in temps]# nearest interpolation won't be better i think
        return temps
    ...
    def conv_relu_unpool(input_filter, output_filter, kernel_size,
        ...
        layers = nn.Sequential(*[
            ...
            nn.Upsample(scale_factor=unpooling_factor, mode='bilinear', align_corners=False),
