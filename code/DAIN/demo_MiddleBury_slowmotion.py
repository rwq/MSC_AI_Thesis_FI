...
# from scipy.misc import imread, imsave deprecated
...
if DO_MiddleBurryOther:
    ...
    for dir in subdir: 
        ...
        print('-----------------------')
        print('SHAPES', [x_.shape for x_ in y_])
        print('-----------------------')
        y_ = [np.transpose(255.0 * item.clip(0,1.0)[:, intPaddingTop:intPaddingTop+intHeight, # REMOVED first 0 index
